<?php 
	...
	if(mysqli_num_rows($slc_query) > 0){
		$row = mysqli_fetch_assoc($slc_query);
			...
			{
				$id = $_POST['id'];
				$dlt_img_name = $_POST['name'];
				$path = "../../uploads/microsite/$dlt_img_name";
				$images = $row['win_images'];
				...
				if(file_exists($path)){
					...
					if(in_array($dlt_img_name, $img_array)){
						...
						$sql_img = "UPDATE micro_site_banner SET win_images = '$img_arr' WHERE id = '$id'";
			...
			{	
				...
				if(file_exists($path)){
					...
					if(unlink($path)){
						...
						$sql = "UPDATE micro_site_banner SET win_video = '' WHERE id = '$dlt_video_id'";
