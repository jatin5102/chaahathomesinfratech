<?php 
	...
	// }else{
		...
		for($i = 0; $i < $length; $i++){
			...
			$convert_destination = $_POST['destination'][$i];
			$destination = mysqli_real_escape_string($conn, $convert_destination);
			
			
			$mutli_sql .= "INSERT INTO micro_site_location_list (project_id, destination) VALUES ('$update_id', '$destination');";
