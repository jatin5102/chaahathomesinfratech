...
$id = $_POST['id'];
$sql = "SELECT * FROM workculturegallery WHERE id='$id'";
...
// for($i = 0; $i < $fetchImages.length; $i++){
//     echo $ml = $fetchImages[$i];
// }

// $finalImages = count($fetchImages);
// $multipleImages = '';
// for($i = 0; $i < $finalImages; $i++){
//     $multipleImages .= $fetchImages[$i];
//         echo $multipleImages;
//     }
...
if($images == ''){
    ...
    for($i = 0; $i < $finalImages; $i++){
        ...
        <img src="../admin/uploads/work-culture/'.$fetchImages[$i].'">
...
for($i = 0; $i < $finalVideos; $i++){
    ...
    <video src="../admin/uploads/work-culture/'.$fetchVideos[$i].'" autoplay="" muted=""></video>
