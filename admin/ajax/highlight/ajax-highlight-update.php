<?php 
	...
	$chk_sql = "SELECT * FROM micro_site_highlights WHERE project_id = '$update_id'";
	...
	if(mysqli_num_rows($chk_query) > 0){
		...
		for($u = 0; $u < $update_count; $u++){
			...
			// $update_meter = $_POST['update_meter'][$u];
			$update_destination = mysqli_real_escape_string($conn, $_POST['update_destination'][$u]);
			
			$update_sql .= "UPDATE micro_site_highlights SET project_id = '$update_id', destination = '$update_destination' WHERE id = '$row_update_id';";
		}
